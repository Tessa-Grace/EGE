...
for mask in range(31): # Если берем 31, то сразу исключаем адрес сети и широковещательный
    net1 = ip_network(f'{ip1}/{mask}', False)
    net2 = ip_network(f'{ip2}/{mask}', False)
    if net1.network_address == net2.network_address:
        if ip1 not in (net1.network_address, net1.broadcast_address): # if net1 == net2 and ip1 in net1.hosts() and ip2 in net1.hosts():
